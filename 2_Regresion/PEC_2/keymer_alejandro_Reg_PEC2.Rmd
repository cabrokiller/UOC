...
Podemos corroborar además la misma información si hacemos el modelo que NO incluye la interacción, y luego comparamos ambos modelos con un `anova()`. El valor de $p$ del estadistico $F$ de esta prueba, es el mismo valor de significación de la interacción del modelo original. Esto permite asumir que el modelo más válido es el que contempla la interacción del factor con la variable cualitativa. 
...
Como en el ejemplo anterior, comenzamos realizando dos modelos; un modelo que conseidera la interacción entre el factor `IndMutant` y la variable cuantitativas `Pulses`, y otro que no. En este caso se puede observar que la interacción deja de ser significativa. De esta manera, en el caso del modelo con la variable repuesta trasnfromada, se puede simplificar quitando el término de la interacción. Lo anterior se puede ver de manera visual al realziar el grafico de disperisón en que se puede ver como ambas rectas en este caso, tienen una pendeinte muy similar, y casi paralela. 


```{r}
# grafico en dimensiones orginales.
df %>%
    mutate(
        curv_a = sin(coef(mod_arc_no_int)[1] + coef(mod_arc_no_int)[2] * Pulses)^2 * 100,
        curv_b = sin(coef(mod_arc_no_int)[1] + coef(mod_arc_no_int)[3] + coef(mod_arc_no_int)[2] * Pulses)^2 * 100
    ) %>%
...
```
 ...
 Al graficar las curvas en el espacio original, e posible ver que el modelo se adapta a una relación no lineal de la variable `Pulses` con la variable `Perc_Reacting`. Esta relación se vuelve lineal al utilzar la transformación. 
 
 Con todo lo anterior se podría concluir que la interaccion entre el grupo con la variable de `Pulses` no tendría mayor importancia. Se podría decir que según este modelo, no hay evidencia de que los grupos de gusano difieran en la rapidez en la que se habitúan al esímulo lumínico, sino que mas bien, el grupo de mutante reacciona menos. La diferencia en al habituacón, que se observa en la sección (a) sería un sesgo dado por la distribución de la variable `Perc_Reacting`, que al ser una proporción, se aleja de una distribución normal. 
...
#prediccion de la respueta en la escala originl
sin(ypred)^2 * 100
```
...
```
 ...
 En primer lugar podemos evidenciar de manera visual, que las variables predeictores presentan correlación alta. Por otra parte al valorar los valore de VIF, se puede observar que hay variables con unos valores muy altos que confirman la exitencia de colinearidad que puede ser probelmática al mometo de validar el modelo completo.
...
Una prueba de anova no permite establcer diferencias entre el modelo original y el modelo reducido por el método de `stepwise`, por lo que por parsimonia podemos elegir el modelo reducido. 
...
En cuanto al poder predictivo del modelo reducido, se puede observar que al utilizar la muesra de `test`, el valor del RMSE es menor que el del modelo original.
...
En el caso de los criterios de AIC y de R², podemos observar que para las diferentes combinaciones, el modelo de 8 predictores es que *maximiza* el valor de R² y al mismo tiempo *minimiza* el AIC. 
...
Una primera aproximación permite observar que un modelo de 8 componentes explcia hasta un 98% de la variabilidad de la muestra. Por otra parte, se puede obserar que la reducción de la varainza explicada, cae bastante desde el componente 5 en adelante lo que tambien podría ayuda a determinar el numero ideal de componentes. 
...
Al utilizar una validazión cruzada del modelo, podemos identificar que el RMSE se minimiza en el modelo con 6 componentes. 
...
```
 ...
 Al utiliza los modelos con 8 y con 6 componentes para predecir la repsuetsa en la muestra de `test` podemos corroborar que el modelo con 6 componentes da una mejor solución. 
...
El modelo de 3 componentes explcia hasta un `r round(as.numeric(mod_plr$Xvar[3]), 1)` de la varianza de los datos. En el gráfico se puede observar que la variable que mejor predice con el eje 1 es `P2`
...
```
Finalmente, caluclamos el valor de RMSE que resulta de la aplicacion del modelo de PLS con la muestra de test.
